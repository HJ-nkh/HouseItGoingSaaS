...
def handler(event, context):
    ...
    # try:
        ...
        try:
            ...
            result = serialize_instance({ 'FEMModel': FEMModel, 'forces': s.loadCombinationsFE_discr, 'UR': s.sectionResults })
            session.execute(
                update(simulations_table)
                .where(simulations_table.c.id == simulation_id)
                .values(status='completed', end_time=func.now(), result=json.dumps(result, default=default_handler), encoded_s=encoded_s)
            )
            session.commit()
        ...
        if simulation_id is not None:
            try:
                session.execute(
                    update(simulations_table)
                    .where(simulations_table.c.id == int(simulation_id))
                    .values(status='failed', end_time=func.now(), error=str(outer_e))
                )
                session.commit()
            except Exception:
                pass
        return {'statusCode': 500, 'body': json.dumps({'error': 'Server error', 'message': str(outer_e)})}
    finally:
        try:
            session.close()
        except Exception:
