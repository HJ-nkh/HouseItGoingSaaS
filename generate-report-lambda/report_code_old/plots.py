# -*- coding: utf-8 -*-
"""
Created on Thu Dec  7 15:37:49 2023
...
class ReportPlots:
    ...
    def URmat(self, matrix, row_labels, col_labels, color, projectNumber, section):
        """
    ...
    def staticPlot(self, model, member, highligtedBeamIndex, projectNumber, save=True):
        U = model.U
        X = model.X
        ...
        for i, b in enumerate(member):
            startCoor = model.X[model.T[model.member[i]['consistOfelements']][0][0]]
            endCoor = model.X[model.T[model.member[i]['consistOfelements']][-1][-1]]
        ...
        node_constraints = self.assemble_constraints(no, con)
        ...
        if save:
            name = "statisksystem" + str(highligtedBeamIndex+1)
            self.saveFigure(fig, projectNumber, name)
    ...
    def plotSectionForcesMember(self, s, projectNumber, ls, loadcomb, save=True):

        # Create custom patches for the legend
        red_patch = mpatches.Patch(color='red', alpha=0.5, label='+')
        blue_patch = mpatches.Patch(color='blue', alpha=0.5, label='$-$')

        for i, member in enumerate(s.member_discr):
            ...
            startCoor = s.X_discr[int(s.T_discr[member['consistOfelements']][0][0])]
            endCoor = s.X_discr[int(s.T_discr[member['consistOfelements']][-1][-1])]      
            ...
            for ax, sectionForceType in zip(axs, SFtypes):
                ...
                row[:-1] if i < len(s.T_discr[member['consistOfelements']]) - 1 else row  # Exclude last element for all rows except the last
                for i, row in enumerate(s.T_discr[member['consistOfelements']])
                ]).flatten().astype(int)
            ...
            if save:
                name = "Member" + str(i+1) + loadcomb
                self.saveFigure(fig, projectNumber, name)


    def plotSectionForcesMemberEnvelope(self, s, projectNumber, ls, save=True):
        ...
        for i, member in enumerate(s.member_discr):
            ...
            if save:
                name = "Member" + str(i+1) + "SectionForceEnvelope"
                self.saveFigure(fig, projectNumber, name)
...
### ----------------------------------------------- Helper functions -----------------------------------------------------------
    ...
    def assemble_constraints(self, nodes, constraints):
        result = {}
    ...
    def fillBetweenMemberAndSectionForce(self, ax, startCoor, endCoor, XSFs, sectionForceType):
